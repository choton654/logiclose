...
import { useGetSaleScenarioQuery, useAddSaleScenarioMutation } from '../../services/query';
...
const SaleScenario = ({ dimension }) => {
    ...
    const [title, setTitle] = useState('Sale Scenario')
    const [saleScenarioState, setsaleScenarioState] = useState([
        { isShown: true, name: "year", label: "Year", placeholder: 'Year', value: '' },
        { isShown: true, name: "exitCapRate", label: "Exit Cap Rate", placeholder: 'Exit Cap Rate', value: '' },
        { isShown: true, name: "noi", label: "NOI", placeholder: 'Units', value: '' },
        { isShown: true, name: "saleProceeds", label: 'Sale Proceeds', placeholder: 'Sale Proceeds', value: "" },
        { isShown: false, name: "sellingCosts", label: 'Selling Costs', placeholder: 'Selling Costs', value: '' },
        { isShown: false, name: "debtRepayment", label: "Debt Repayment", placeholder: 'Debt Repayment', value: "" },
        { isShown: false, name: "transferTax", label: 'Transfer Tax', placeholder: 'Transfer Tax', value: '' },
        { isShown: false, name: "salePrice", label: "Sale Price", placeholder: 'Sale Price', value: "" },
    ...
    const toggleTitle = () => {
        if (title.trim() === '') {
            setTitle('Sale Scenario')
        }
        setIsTitleEdit(!isTitleEdit)
    }

    const handleChange = (idx, text) => {
        let newFieldsData = [...saleScenarioState]
        newFieldsData[idx].value = text
        setsaleScenarioState(newFieldsData)
    }

    const toggleLabel = (idx) => {
        if (saleScenarioState[idx]['label'].trim() === '') {
            const newFIelds = [...saleScenarioState]
            newFIelds[idx]['label'] = 'Label'
            setsaleScenarioState(newFIelds)
        }
        setSubtitleId(idx)
        setTimeout(() => {
            if (idx === subTitleId) {
    ...
    const { data: saleData, isLoading: getting, error: summaryError } = useGetSaleScenarioQuery(token)
    const [submitSaleScenario, { isLoading, error, data }] = useAddSaleScenarioMutation()

    const handleSubmit = useCallback(() => {
        const isFieldEmpty = saleScenarioState.slice(0, 2).some(item => item.value.trim() === '')
        const reqData = saleScenarioState.reduce((prev, curr) => {
            const newItem = {
                ...prev,
                [curr.name]: {
                    subtitle: curr.label,
        ...
        let saleScenarioData = {}
        if (id) {
            saleScenarioData = { reqData, title, isStepCompleted: true, id }
        } else {
            saleScenarioData = { reqData, title, isStepCompleted: true }
        }
        submitSaleScenario({ saleScenarioData, token })
    }, [useAddSaleScenarioMutation, title, saleScenarioState])

    useEffect(() => {
        setNumberOfActiveFields(saleScenarioState.filter(item =>
            item.isShown).length)
    }, [saleScenarioState])

    useEffect(() => {
        if (saleData !== undefined) {
            const { createdAt, updatedAt, __v, _id, userId, isStepCompleted, title, ...rest } = saleData
            ...
            exitScenariosSteps[1].completed = isStepCompleted
            setsaleScenarioState(newRefinanceArr)
            setTitle(saleData.title)
            setId(saleData._id)
            let cacheSteps = JSON.parse(getData('stepCompleted'))
            cacheSteps.map(item => {
                if (item.label === exitScenariosSteps[1].label) {
                    item.completed = saleData.isStepCompleted
                }
            })
    ...
    return (
        <LogiCLoseLayout
            ...
            isLoading={isLoading}
            goBackRoute={'/exitScenarios/refinanceScenario'}
        >
            {getting ?
                <Box sx={{
                ...
                <Paper sx={{
                    ...
                    <Grid container sx={{ mt: 3, borderColor: "#424242" }}>
                        {saleScenarioState.map((field, idx) =>
                            field.isShown &&
                            <Grid key={idx}
                                item
                                xs={12}
            ...
            >
                <Fade in={open}>
                    <Box sx={{
                        ...
                        {numberOfActiveFields !== 9 ?
                            <Box>
                                {[...saleScenarioState].slice(4, saleScenarioState.length).map((item, idx) =>
                                    !item.isShown &&
                                    <Box key={idx} sx={{
                                        cursor: "pointer",
...
export default IsAuthHOC(SaleScenario, "/exitScenarios/saleScenario");