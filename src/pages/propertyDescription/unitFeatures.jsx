...
import {
    ...
    Paper, Divider, Grid, CircularProgress, Fade
} from '@mui/material';
import Tooltip, { tooltipClasses } from '@mui/material/Tooltip';
import { styled } from '@mui/material/styles';
import summaryPreview from "../../../public/WebView/UnitfeaturePreview.png"
...
import { useAddunitFeaturesMutation, useGetUnitFeatureQuery, useUpdateUnitFeatureMutation } from '../../services/query';
import IsAuthHOC from '../../components/IsAuthHOC';
import CircularProgressWithLabel from '../../components/CircularLoader';
import PreviewModal from '../../components/PreviewModal';
import unitFeature1 from "../../../public/WebView/Unitfeature1.png"
import unitFeature2 from "../../../public/WebView/Unitfeature2.png"
import unitFeature3 from "../../../public/WebView/Unitfeature3.png"
...
const PreviewChild = () =>
    <Box className='flex flex-row justify-between my-6 px-6'>
        ...
        <Box className='flex flex-row w-2/5'>
            <Box className='flex flex-col' sx={{ mr: 2, width: 250 }}>
                <Image src={unitFeature2} />
                <Box sx={{ mt: 3 }}>
                    <Image src={unitFeature1} />
                </Box>
            </Box>
            <Box sx={{ width: 260 }}>
                <Image src={unitFeature3} />
            </Box>
        </Box>
    </Box>

const UnitFeatures = ({ dimension }) => {
    ...
    const [title, setTitle] = useState('Unit Features')
    ...
    const removeImages = (idx, imgidx) => {
        const newunitFeatures = [...unitFeatures]
        newunitFeatures[idx].images[imgidx].preview = undefined
        // newunitFeatures[idx].images.splice(imgidx, 1)
        setUnitFeatures(newunitFeatures)
    }

    useEffect(() => {
        if (files.length > 0) {
            const newFIelds = [...unitFeatures]
    ...
    const handleChange = (idx, e) => {
        const newFIelds = [...unitFeatures]
        newFIelds[idx]['subTitleText'] = e.target.value
        setUnitFeatures(newFIelds)
    }
    ...
    const removeTestField = (idx) => {
        let newFieldsData = [...unitFeatures]
        newFieldsData.splice(idx, 1)
        setUnitFeatures(newFieldsData)
    }

    const toggleSubTitle = (idx) => {
        if (unitFeatures[idx]['subTitle'].trim() === '') {
            const newFIelds = [...unitFeatures]
            newFIelds[idx]['subTitle'] = 'Feature 1'
            setUnitFeatures(newFIelds)
        }
        setSubtitleId(idx)
        setTimeout(() => {
    ...
    const { data: unitFeatureData, isLoading: getting, error: getError } = useGetUnitFeatureQuery(token)
    const [updateUnitFeature, { isLoading: updating, error: updateError, data: updatedData }] = useUpdateUnitFeatureMutation()
    const [submitUnitFeatures, { isLoading, error, data }] = useAddunitFeaturesMutation()

    const handleSubmit = useCallback(() => {
        let formData = new FormData()
        formData.append('title', title)
        formData.append('noOfFiles', JSON.stringify(unitFeatures.map(text => text.noOfFiles)))
        formData.append('noOfNewFiles', JSON.stringify(unitFeatures.map(text => text.noOfNewFiles)))
        formData.append('subTitleArr', JSON.stringify(unitFeatures.map(text => text.subTitle)))
        formData.append('subTitleTextArr', JSON.stringify(unitFeatures.map(text => text.subTitleText)))
        let newImg = []
        unitFeatures.forEach(item => {
        ...
        const isEmptySubtitle = unitFeatures.some(item => item.subTitle.trim() === '')
        // const isEmptySubtitleText = unitFeatures.some(item => item.subTitleText.trim() === '')
        // if (isEmptySubtitle) {
        ...
        // }
        if (!id) {
            submitUnitFeatures({ formData, token })
        } else {
            formData.append('id', id)
            updateUnitFeature({ formData, token })
        }

    }, [useAddunitFeaturesMutation, useUpdateUnitFeatureMutation, title, unitFeatures])

    useEffect(() => {
        if (unitFeatureData !== undefined) {
            // console.log(unitFeatureData);
            setUnitFeatures(unitFeatureData.unitFeatures.map(item => ({
            ...
            propertySumarySteps[2].completed = unitFeatureData.isStepCompleted
            setTitle(unitFeatureData.title)
            setId(unitFeatureData._id)
            let cacheSteps = JSON.parse(getData('stepCompleted'))
            cacheSteps.map(item => {
                if (item.label === propertySumarySteps[2].label) {
                    item.completed = unitFeatureData.isStepCompleted
                }
            })
    ...
    useEffect(() => {
        if (data !== undefined || updatedData !== undefined) {
            if (!saveContent) {
                setStepCount(prevStep => prevStep + 1)
                router.push("/propertyDescription/floorPlans")
    ...
    return (
        <LogiCLoseLayout
            ...
            stepCount={stepCount}
            goBackRoute={'/propertyDescription/communityFeature'}
        >
            {getting ?
                <Box sx={{
                ...
                <Paper sx={{
                    ...
                    {unitFeatures.map((item, idx) =>
                        <Box key={idx} sx={{ mt: idx !== 0 ? 3 : 0 }}>
                            <Box sx={{ display: "flex", flexDirection: "column", width: isMobileDevice() ? "100%" : "50%" }}>
                                ...
                                <input className='w-full mt-1 border-2 border-stone-300 bg-[#fff] h-10 rounded-md px-2 font-normal'
                                    placeholder='Unit feature descriptioon'
                                    size='small'
                                    type="text"
                                    value={item.subTitleText}
                                    onChange={(e) => handleChange(idx, e)}
                                    style={{ fontSize: 14, marginTop: 2, fontFamily: "Poppins" }}
                                />
                            </Box>
                            <Box sx={{ mt: 2 }}>
                                <Typography variant='subtitle1'
                                    sx={{ fontFamily: "Poppins", width: dimension && dimension.innerWidth > 600 ? 200 : 100 }}>Images</Typography>
                                <Grid container component="main" sx={{ background: "#fff", cursor: "pointer" }}>
                                    <div {...getRootProps({
                                        style,
                    ...
                    <Box className="flex flex-row justify-between">
                        <Box onClick={() => setUnitFeatures(prev => [...prev,
                        { subTitle: `Unit Feature ${unitFeatures.length + 1}`, images: [], noOfNewFiles: 0 }])} sx={{ cursor: "pointer", display: "flex", alignItems: "center" }}>
            ...
            >
                <Alert onClose={closeSnackBar} severity="error" sx={{ width: '100%' }}>
                    {error !== undefined ? error.data?.message :
                        imageFieldEmpty ? "Text field can't be empty"
...
export default IsAuthHOC(UnitFeatures, "/propertyDescription/unitFeatures");
