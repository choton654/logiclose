...
// import summaryPreview from "../../../public/assets/LocationSummaryPreview.png"
...
import { useAddLocationSummaryMutation, useGetLocationSummaryQuery } from '../../services/query';
import IsAuthHOC from '../../components/IsAuthHOC';
import CircularProgressWithLabel from '../../components/CircularLoader';
import summaryPreview from "../../../public/WebView/LocationSummary.png"
...
const PreviewChild = () => <Box className='flex flex-col mt-4 py-6 pl-6'>
    ...
    <Box className='flex flex-row mt-4 justify-between relative duration-500'
        sx={{ height: '70vh' }}
    >
        ...
        <Box sx={{ width: '96%', borderWidth: 1, position: 'absolute', top: 100, left: 0 }}>
            <Image src={locationOverview} layout='responsive' />
        </Box>
    </Box>
</Box>

const LocationSummary = ({ dimension }) => {
    ...
    const [title, setTitle] = useState('Location Summary')
    ...
    const removeImages = (idx, imgidx) => {
        const newlocationSummary = [...locationSummary]
        newlocationSummary[idx].images[imgidx].preview = undefined
        setlocationSummary(newlocationSummary)
    }

    useEffect(() => {
        if (files.length > 0) {
            const newFIelds = [...locationSummary]
    ...
    const removeTestField = (idx) => {
        let newFieldsData = [...locationSummary]
        newFieldsData.splice(idx, 1)
        setlocationSummary(newFieldsData)
    }

    const toggleSubTitle = (idx) => {
        if (locationSummary[idx]['subTitle'].trim() === '') {
            const newFIelds = [...locationSummary]
            newFIelds[idx]['subTitle'] = 'Label 1'
            setlocationSummary(newFIelds)
        }
        setSubtitleId(idx)
        setTimeout(() => {
    ...
    const { data: locationSummaryData, isLoading: getting, error: getError } = useGetLocationSummaryQuery(token)
    const [submitlocationSummary, { isLoading, error, data }] = useAddLocationSummaryMutation()

    const handleSubmit = useCallback(() => {
        let formData = new FormData()
        formData.append('title', title)
        formData.append('noOfFiles', JSON.stringify(locationSummary.map(text => text.noOfFiles)))
        formData.append('noOfNewFiles', JSON.stringify(locationSummary.map(text => text.noOfNewFiles)))
        formData.append('subTitleArr', JSON.stringify(locationSummary.map(text => text.subTitle)))
        formData.append('subTitleTextArr', JSON.stringify(locationSummary.map(text => text.subTitleText)))
        let newImg = []
        ...
        const isEmptySubtitle = locationSummary.some(item => item.subTitle.trim() === '')
        ...
        submitlocationSummary({ formData, token })

    }, [useAddLocationSummaryMutation, title, locationSummary])

    useEffect(() => {
        if (locationSummaryData !== undefined) {
            // console.log(locationSummaryData);
            setlocationSummary(locationSummaryData.locationSummary.map(item => ({
            ...
            locationOverViewSteps[0].completed = locationSummaryData.isStepCompleted
            setTitle(locationSummaryData.title)
            setId(locationSummaryData._id)
            let cacheSteps = JSON.parse(getData('stepCompleted'))
            cacheSteps.map(item => {
                if (item.label === locationOverViewSteps[0].label) {
                    item.completed = locationSummaryData.isStepCompleted
                }
            })
    ...
    useEffect(() => {
        if (data !== undefined) {
            if (!saveContent) {
                ...
                router.push("/locationOverView/employers")
            } else {
                setId(data._id)
            }
        }
    ...
    return (
        <LogiCLoseLayout
            dimension={dimension}
            layoutHeight={'100%'}
            // layoutHeight={isMobileDevice() || locationSummary.length > 1 ? "100%" : "110vh"}
            title={"Location Overview"}
            insideSteps={locationOverViewSteps}
            stepCount={stepCount}
            goBackRoute={'/propertyDescription/comps'}
        >
            {getting ?
                <Box sx={{
                ...
                <Paper sx={{
                    ...
                    <Box sx={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        {/* <Box onClick={() => setlocationSummary(prev => [...prev,
                        { subTitle: `Label ${locationSummary.length + 1}`, images: [], noOfNewFiles: 0 }])} sx={{ cursor: "pointer", display: "flex", alignItems: "center" }}>
                    ...
                    {locationSummary.map((item, idx) =>
                        <Box key={idx} sx={{ mt: idx !== 0 ? 3 : 0 }}>
                            <Box sx={{ display: "flex", flexDirection: "column", width: '100%' }}>
                                ...
                                <textarea className='w-full mt-1 border-2 border-stone-300 bg-[#fff] h-[10] rounded-md px-2 py-2 font-normal'
                                    style={{ fontFamily: "Poppins", height: 100 }}
...
export default IsAuthHOC(LocationSummary, "/locationOverView/locationSummary");
