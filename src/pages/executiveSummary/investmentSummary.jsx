...
import {
    ...
    Paper, CircularProgress, Alert, Fade, Backdrop, Modal
} from '@mui/material';
...
import {
    useAddInvestmentSummaryMutation, useGetInvestmentSummaryQuery,
...
import summaryPreview from "../../../public/WebView/SummaryPreview.png"
...
const PreviewChild = () => <Box className='flex flex-col rounded-3xl my-6 mx-4 p-6 bg-gray-100'>
...
const InvestmentSummary = ({ dimension }) => {
    ...
    const [submitInvestmentSummer, { isLoading, error, data }] = useAddInvestmentSummaryMutation()
    const [updateInvestmentSummer, { isLoading: updating, error: updateError, data: updateData }] = useUpdateInvestmentSummaryMutation()
    ...
    const handleSubmit = useCallback(() => {
        ...
        if (!id) {
            submitInvestmentSummer({ formData, token })
        } else {
            formData['id'] = id
            updateInvestmentSummer({ formData, token })
        }

    }, [useAddInvestmentSummaryMutation, useUpdateInvestmentSummaryMutation, title, noOfTextFields])

    useEffect(() => {
        if (summaryData !== undefined) {
            const newDescriptionArr = summaryData.descriptionArr.map(dsc =>
                ({ description: dsc })
            )
            executiveSummarySteps[0].completed = summaryData.isStepCompleted
            ...
            cacheSteps.map(item => {
                if (item.label === executiveSummarySteps[0].label) {
    ...
    useEffect(() => {
        if ((data !== undefined || updateData !== undefined) && !saveContent) {
            ...
            router.push("/executiveSummary/investmentOppurtunity")
    ...
    }, [data, error, updateError, updateData, saveContent])
        ...
        >
                    ...
                    {noOfTextFields.map((item, idx) =>
                        <Box key={idx} sx={{
                            ...
                            <textarea className='w-full mt-1 border-2 border-stone-300 bg-[#fff] h-100 rounded-md px-2 py-2 font-normal'
                                ...
                                placeholder="Enter Investment Summar"
                                type="text"
                                name='description'
                            ...
                            {idx !== 0 &&
                                <svg
                                    onClick={() => removeField(idx, noOfTextFields, setNoOfTextFields)}
            ...
            >
                <Alert onClose={closeSnackBar} severity="error" sx={{ width: '100%' }}>
                    {error !== undefined ? error.data?.message : 'You must enter first summary description'}
                </Alert>
            </Snackbar>
            <PreviewModal wrapperComponent={PreviewChild}
                open={open} handleClose={handleClose}
                imgSrc={summaryPreview} />
            {/* <PreviewModal open={open} handleClose={handleClose} imgSrc={summaryPreview} /> */}
        </LogiCLoseLayout>
    );
}

export default IsAuthHOC(InvestmentSummary, '/executiveSummary/investmentSummary');
