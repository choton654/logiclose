...
import summaryPreview from "../../../public/WebView/SourceFundPreview.png"
...
import { useAddSourceFundMutation, useGetSourceFundQuery } from '../../services/query';
...
const SourceFund = ({ dimension }) => {
    ...
    const [title, setTitle] = useState('Sources/Uses')
    const [sourceFundState, setsourceFundState] = useState([
        { isShown: true, name: "purchasePrice", label: "Purchase Price", placeholder: '$', value: '' },
        { isShown: true, name: "loanAmount", label: "Loan Amount", placeholder: '$', value: '' },
        { isShown: true, name: "equity", label: "Equity", placeholder: '$', value: '' },
        { isShown: true, name: "syndicationFee", label: "Syndication Fee", placeholder: '$', value: '' },
        { isShown: true, name: "capex", label: 'Capex', placeholder: '$', value: '' },
        { isShown: true, name: "closingCosts", label: "Closing Costs", placeholder: '$', value: '' },
    ...
    const toggleTitle = () => {
        if (title.trim() === '') {
            setTitle('Sources/Uses')
        }
        setIsTitleEdit(!isTitleEdit)
    }

    const handleChange = (idx, text) => {
        let newFieldsData = [...sourceFundState]
        ...
        setsourceFundState(newFieldsData)
    }

    const toggleLabel = (idx) => {
        if (sourceFundState[idx]['label'].trim() === '') {
            const newFIelds = [...sourceFundState]
            newFIelds[idx]['label'] = 'Label'
            setsourceFundState(newFIelds)
        }
        setSubtitleId(idx)
        setTimeout(() => {
            if (idx === subTitleId) {
    ...
    const [submitSourceFund, { isLoading, data, error }] = useAddSourceFundMutation()
    const { data: sourceFundData, isLoading: getting, error: getError } = useGetSourceFundQuery(token)

    const handleSubmit = useCallback(() => {
        const isFieldEmpty = sourceFundState.slice(0, 2).some(item => !item.value)
        const reqData = sourceFundState.reduce((prev, curr) => {
            const newItem = {
                ...prev,
                [curr.name]: {
                    subtitle: curr.label,
        ...
        let sourceFundFormData = {}
        if (id) {
            sourceFundFormData = { reqData, title, isStepCompleted: true, id }
        } else {
            sourceFundFormData = { reqData, title, isStepCompleted: true }
        }
        submitSourceFund({ sourceFundFormData, token })
    }, [useAddSourceFundMutation, title, sourceFundState])

    useEffect(() => {
        setNumberOfActiveFields(sourceFundState.filter(item =>
            item.isShown).length)
    }, [sourceFundState])

    useEffect(() => {
        if (sourceFundData !== undefined) {
            const { createdAt, updatedAt, __v, _id, userId, isStepCompleted, title, ...rest } = sourceFundData
            ...
            financialSummarySteps[0].completed = isStepCompleted
            setsourceFundState(newRefinanceArr)
            setTitle(sourceFundData.title)
            setId(sourceFundData._id)
            let cacheSteps = JSON.parse(getData('stepCompleted'))
            cacheSteps.map(item => {
                if (item.label === financialSummarySteps[0].label) {
                    item.completed = sourceFundData.isStepCompleted
                }
            })
    ...
    useEffect(() => {
        if (data !== undefined) {
            if (!saveContent) {
                ...
                router.push("/financialSummary/closingCapital")
            } else {
                setId(data._id)
            }
        }
    ...
    return (
        <LogiCLoseLayout
            ...
            stepCount={stepCount}
            goBackRoute={'/locationOverView/employers'}
        >
            {getting ?
                <Box sx={{
            ...
            >
                <Fade in={open}>
                    <Box sx={{
                        ...
                        {numberOfActiveFields !== 8 ?
                            <Box>
                                {[...sourceFundState].slice(2, sourceFundState.length).map((item, idx) =>
                                    !item.isShown &&
                                    <Box key={idx} sx={{
                                        cursor: "pointer",
...
export default IsAuthHOC(SourceFund, "/financialSummary/sourceFund");

   // const router = useRouter()
    // const [stepCount, setStepCount] = useState(0)
    // const token = getData('token')
    // const [id, setId] = useState(null)
    // const [title, setTitle] = useState('SOURCE / USE OF FUNDS')
    // const [isTitleEdit, setIsTitleEdit] = useState(false)
    // const [isSubTitleEdit, setIsSubTitleEdit] = useState(false)
    // const [subTitleId, setSubtitleId] = useState(0)
    // const [sourceFundState, setsourceFundState] = useState([{
    ...
    // const toggleTitle = () => {
    //     if (title.trim() === '') {
    //         setTitle('SOURCE / USE OF FUNDS')
    //     }
    //     setIsTitleEdit(!isTitleEdit)
    // }

    // const toggleSubTitle = (idx) => {
    //     if (sourceFundState[idx]['subTitle'].trim() === '') {
    //         const newFIelds = [...sourceFundState]
    //         newFIelds[idx]['subTitle'] = 'SOURCE / USE OF FUNDS'
    //         setsourceFundState(newFIelds)
    //     }
    //     setSubtitleId(idx)
    //     setTimeout(() => {
    ...
    //     sourceFundState.forEach((item, idx) => {
    ...
    //         return
    //     }
    //     const formData = { reqData: sourceFundState, title }
    //     if (id) {
    //         formData['id'] = id
    //     }
    //     submitSourceFund({ formData, token })
    // }, [useAddSourceFundMutation, title, sourceFundState])

    // useEffect(() => {
    //     if (sourceFundData !== undefined) {
    //         setsourceFundState(sourceFundData.featureData.map(item => ({
    //             subTitle: item.subTitle,
    //             description: item.description
    //         })))
    //         setTitle(sourceFundData.title)
    //         setId(sourceFundData._id)
    //         financialSummarySteps[0].completed = sourceFundData.isStepCompleted
    //         let cacheSteps = JSON.parse(getData('stepCompleted'))
    //         cacheSteps.map(item => {
    //             if (item.label === financialSummarySteps[0].label) {
    //                 item.completed = sourceFundData.isStepCompleted
    ...
    //     <LogiCLoseLayout
    //         dimension={dimension}
    //         layoutHeight={sourceFundState.length > 2 ? "100%" : "100vh"}
    //         title={"Financial Summary"}
    //         insideSteps={financialSummarySteps}
    //         stepCount={stepCount}
    //         goBackRoute={'/locationOverView/employers'}
    ...
    //                                 </svg>
    //                             </Box>
    //                             <Box onClick={() => setsourceFundState(prev => [...prev,
    //                             { subTitle: `Label ${sourceFundState.length + 1}`, description: '' }])}
